...
func NewListCacheActorCluster(clusterName string, nodeNumber int) (*actor.PID, *BroadcastStopGroup, *BroadcastStringKeysGroup) {
	var nodes = make([]*actor.PID, nodeNumber)
	for i := 0; i < nodeNumber; i++ {
		nodes[i] = NewListCacheActor(clusterName, fmt.Sprintf("lists%d", i))
	}
